...
class OrgApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Отделы и сотрудники (удаление по кабинету)")
        self.root.geometry("800x550")
        self.root.resizable(True, True)

        # Данные теперь хранятся в атрибутах экземпляра
        self.departments = [
    ...
    def create_widgets(self):
        ...
        ttk.Button(control_frame, text="Удалить сотрудников выбранного отдела",
                   command=self.delete_employees_by_department).pack(side="left", padx=10)
        ...
        self.status_label = ttk.Label(self.root, text="Выберите отдел, затем нажмите кнопку удаления сотрудников", relief="sunken", anchor="w")
    ...
    def on_department_selected(self, event):
        ...
        for dept in self.departments:
            if dept["name"] == dept_name:
                ...
                self.status_label.config(text=f"Выбран отдел '{dept_name}', кабинет {dept['room']}")
                break

    def delete_employees_by_department(self):
        ...
        to_delete = [emp for emp in self.employees if emp["room"] == room]
        if not to_delete:
        ...
        answer = messagebox.askyesno("Подтверждение",
                                     f"Удалить {len(to_delete)} сотрудников отдела '{self.current_department['name']}'?")
        if not answer:
            return
        self.employees = [emp for emp in self.employees if emp["room"] != room]
        self.refresh_employees()
        self.status_label.config(text=f"Удалено {len(to_delete)} сотрудников из кабинета {room}")
